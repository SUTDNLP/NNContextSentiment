/*
 * CNNHSWordClassifier.h
 *
 *  Created on: Mar 18, 2015
 *      Author: mszhang
 */

#ifndef SRC_CNNHSWordClassifier_H_
#define SRC_CNNHSWordClassifier_H_
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, const NRMat<dtype>& wordhEmb, int wordcontext, int labelSize, int wordHiddenSize, int hiddenSize) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        for (int idy = 0; idy < word_num; idy++) {
          if(idx == seq_size - 1) _words.GetEmb(words[idy], wordprime[idx][idy]);
          else _hwords.GetEmb(words[idy], wordprime[idx][idy]);
        }
...
#endif /* SRC_CNNHSWordClassifier_H_ */
